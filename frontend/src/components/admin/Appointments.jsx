...
const Appointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const updatedData = await Promise.all(
        data.map(async (appt) => {
          const [username, hospitalName, doctorName, equipmentName] =
            await Promise.all([
  ...
  const updateStatus = async (id, newStatus, userId) => {
    setAppointments((prev) =>
      prev.map((appt) =>
        appt.appointment_id === id ? { ...appt, status: newStatus } : appt
      )
    );
                ...
                <tr key={appt.appointment_id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4 border-b flex gap-2">
                    <button
                      ...
                      className="px-2 py-1 rounded bg-red-500 text-white hover:bg-black"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => updateStatus(appt.appointment_id, "Confirmed", appt.user_id)}
                      className="px-2 py-1 rounded bg-blue-500 text-white hover:bg-black"
                    >
                      Confirm
                    </button>
                  </td>
